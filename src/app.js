...
<div class="container" style="margin-top: ${UI.header_padding}px; margin-bottom: 60px;">
	<div class="row align-items-start g-3">
		`;
  if (
    !window.location.href.toLowerCase().includes(':search?q=') &&
    ...
    if (navarray.length > 0) {
      for (var i in navarray) {
        ...
        var decodedPathPart = decodeURIComponent(pathPart).replace(
          /\//g,
          '%2F'
        );
        var trimmedPathPart = decodedPathPart.replace(/\?.+/g, "$'");
        var displayedPathPart =
          trimmedPathPart.length > 50
            ? trimmedPathPart.slice(0, 50) + '...'
            : trimmedPathPart.slice(0, 50);
...
<footer class="footer text-center mt-auto container ${
      UI.footer_style_class
    }" style="${UI.fixed_footer ? 'position: fixed;' : ''} ${
    ...
    <div class="container" style="padding-top: 15px;">
      ...
      UI.company_link
    }" target="_blank">${UI.company_name}</a> with ❤️
      ${
        UI.credit
      ...
      <a href="${
        UI.contact_link
      }" title="Please allow us up to 48 hours to process DMCA requests.">DMCA</a> ∙ <a href="${
      UI.contact_link
    }">Contact</a>
      </div>
      <div class="col-lg-4 col-md-12 text-lg-end">
        <p>
          <a href="#"><img src="https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2F` +
...
function title(path) {
  ...
  else $('title').html(`${drive_name}: ${path} - ${UI.siteName}`);
}
...
function nav(path) {
  ...
  } ${UI.header_style_class} container">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
      <li class="nav-item">
        <a class="nav-link" href="/${cur}:/"><i class="fas fa-home fa-fw"></i>${
    UI.nav_link_1
  }</a>
      </li>`;
  ...
  html +=
    `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle d-flex align-items-center gap-2" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">` +
    gdrive_icon +
    ` ${drive_name}</a><div class="dropdown-menu" aria-labelledby="navbarDropdown">`;
  names.forEach((name, idx) => {
    html +=
      `<a class="dropdown-item d-flex align-items-center gap-2"  href="/${idx}:/">` +
      gdrive_icon +
      ` ${name}</a>`;
  });
  html += `</div></li>`;
  ...
  html += `<li class="nav-item">
    <a class="nav-link" href="${
      UI.contact_link
    }" target="_blank"><i class="fas fa-paper-plane fa-fw"></i>${
    UI.nav_link_4
  }</a>
  </li>${
  ...
  try {
    ...
    for (i in files) {
      ...
      if (item['mimeType'] == 'application/vnd.google-apps.folder') {
        html += `<div class="list-group-item list-group-item-action d-flex align-items-center flex-md-nowrap flex-wrap justify-sm-content-between column-gap-2"><a href="${p}" style="color: ${
          UI.folder_text_color
        };" class="countitems w-100 d-flex align-items-start align-items-xl-center gap-2"><span>${folder_icon}</span>${
          item.name
        }</a>${
          UI.display_time
            ? `<span class="badge bg-info" style="margin-left: 2rem;">` +
              item['createdTime'] +
              `</span>`
            : ``
        }${
          UI.display_size
            ? `<span class="badge bg-primary my-1 text-center" style="min-width: 85px;">—</span>`
            : ``
        }<span class="d-flex gap-2">
				${
          UI.display_drive_link
            ? `<a class="d-flex align-items-center" href="https://sharer.winten.my.id/f/${item['fid']}" target="_blank" title="via Google Drive">${gdrive_icon}</a>`
            : ``
        }
				${
          UI.display_download
        ...
        c += ' view';
        //}
        html += `<div class="list-group-item list-group-item-action d-flex align-items-center flex-md-nowrap flex-wrap justify-sm-content-between column-gap-2">${
          UI.allow_selecting_files
            ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="' +
              link +
              '" id="flexCheckDefault">'
            : ''
        }<a class="countitems size_items w-100 d-flex align-items-start align-items-xl-center gap-2" style="text-decoration: none; color: ${
          UI.css_a_tag_color
        };" href="${p}&a=view"><span>`;
        ...
        html += `</span>${item.name}</a>${
          ...
          UI.display_size
            ? `<span class="badge bg-primary my-1 ${
                item['size'] == '—' ? 'text-center' : 'text-end'
    ...
    if (is_lastpage_loaded) {
      ...
      if (total_files == 0) {
        ...
        $('#count')
          .removeClass('d-none')
          .find('.totalsize')
          .text(total_files + ' file, total: ' + total_size);
      } else {
        $('#count')
          .removeClass('d-none')
          .find('.totalsize')
          .text(total_files + ' files, total: ' + total_size);
      }
    }
  } catch (e) {
    console.log(e);
...
function append_search_result_to_list(files) {
  try {
    ...
    for (i in files) {
      ...
      item['createdTime'] = utc2jakarta(item['createdTime']);
      if (item['mimeType'] == 'application/vnd.google-apps.folder') {
        html += `<div class="list-group-item list-group-item-action d-flex align-items-center flex-md-nowrap flex-wrap justify-sm-content-between column-gap-2"><a href="#" class="countitems w-100 d-flex align-items-start align-items-xl-center gap-2" style="color: ${
          UI.folder_text_color
        };" onclick="onSearchResultItemClick('${
          item['id']
        }', false, ${JSON.stringify(item).replace(
          /"/g,
          '&quot;'
        )})" data-bs-toggle="modal" data-bs-target="#SearchModel"><span>${folder_icon}</span>${
          item.name
        }</a> ${
          UI.display_time
            ? `<span class="badge bg-info" style="margin-left: 2rem;">` +
          ...
          UI.display_drive_link
            ...
            : ``
				...
				${
          UI.display_download
            ? `<a class="d-flex align-items-center" href="#" title="via Index" onclick="onSearchResultItemClick('${
                item['id']
              }', false, ${JSON.stringify(item).replace(
                /"/g,
                '&quot;'
              )})" data-bs-toggle="modal" data-bs-target="#SearchModel"><i class="far fa-folder-open fa-lg"></i></a>`
            : ``
        }</span></div>`;
      } else {
        var is_file = true;
        ...
        html += `<div class="list-group-item list-group-item-action d-flex align-items-center flex-md-nowrap flex-wrap justify-sm-content-between column-gap-2" gd-type="$item['mimeType']}">${
          UI.allow_selecting_files
            ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="' +
              ...
              '" id="flexCheckDefault">'
            : ''
        }<a href="#" onclick="onSearchResultItemClick('${
          item['id']
        }', true, ${JSON.stringify(item).replace(
          /"/g,
          '&quot;'
        )})" data-bs-toggle="modal" data-bs-target="#SearchModel" class="countitems size_items w-100 d-flex align-items-start align-items-xl-center gap-2" style="text-decoration: none; color: ${
          UI.css_a_tag_color
        };"><span>`;
    ...
    .then(function (obj) {
      ...
      btn =
        ...
        `
				<a href="${encodedUrl}${
          can_preview ? '?a=view' : ''
        }" type="button" class="btn btn-success" target="_blank"><i class="fas fa-bolt fa-fw"></i>Index</a>
				</div>` +
        close_btn;
      $('#modal-body-space').html(content);
    ...
    `
			...
			<div class="col-md-12">
				<div class="text-center">
					...
					<div class="btn-group text-center"> ${
            UI.display_drive_link
              ? ` <a class="btn btn-secondary d-flex align-items-center gap-2" href="https://sharer.winten.my.id/f/${file_id}" id="file_drive_link" target="_blank">` +
                gdrive_icon +
                `Google Drive</a>`
              : ``
  ...
  if (poster && !mimeType.startsWith('application/vnd.google-apps')) {
    ...
    $(img)
      .on('load', function () {
        ...
        $('#preview').css({
          background: 'url("' + poster + '") 0 0 / 100% 100% no-repeat',
        });
...
function file_audio(
		...
		<div class="card-body row g-3">
			<div class="col-lg-4 col-md-12">
				<div class="h-100 border border-dark rounded" style="--bs-border-opacity: .5;">
					${
            UI.disable_player
              ? `<img class="object-fit-cover w-100 h-100 img-fluid rounded" src="${UI.audioposter}" alt="Thumbnail of ${name}" title="Thumbnail of ${name}">`
              : player
          }
				</div>
			</div>
  ...
  var videoJsScript = document.createElement('script');
  videoJsScript.src =
    'https://vjs.zencdn.net/' + player_config.videojs_version + '/video.min.js';
  videoJsScript.onload = function () {
    // Video.js is loaded, initialize the player
  ...
  videoJsStylesheet.href =
    'https://vjs.zencdn.net/' + player_config.videojs_version + '/video-js.css';
  videoJsStylesheet.rel = 'stylesheet';
  document.head.appendChild(videoJsStylesheet);
}
